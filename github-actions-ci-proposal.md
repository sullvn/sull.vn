...
Reviewed 8 September 2026 · Revised 9 September 2026 after a second review · Revised 12 September 2026 after a third review · Proposal only · Temporary review document; remove before merging
...
The earlier 490 MiB estimate was about half the real size. Chromium is the 214 MiB difference between the two rows, downloaded on every run for a tool CI never executes. Keep one shared shell for now; if measured runtime warrants trimming, a CI shell built with `inputsFrom` that omits Chromium is the first cut. actionlint brings in Python for its Pyflakes integration; the pinned package has no option to omit it, and the size is acceptable. [Repository flake](https://github.com/sullvn/sull.vn/blob/9836e696c3a0c072968296ecc4dbb17a663427ac/flake.nix), [locked Playwright package](https://github.com/NixOS/nixpkgs/blob/42f17a57f4f6e33b3de3dca0a2a5ea5233169d02/pkgs/development/web/playwright/driver.nix)
...
Playwright’s console fallback in CI is the terse dot reporter; the `github` reporter is the documented choice for Actions and combines with the HTML reporter. Keep Playwright’s default worker count: six cases on four vCPUs do not need serializing, and a CI-only `workers` override is not worth its branch unless the first runs show resource flakiness. Preserve build-before-preview startup, the preview port, the `--ignore-lock` flag, the foreground-process environment setting, `reuseExistingServer: false`, and all six projects. The environment setting suppresses Astro 7’s agent auto-background mode and the flag skips the preview lock file, so Playwright keeps control of the server. The fixed port and `reuseExistingServer: false` also mean two worktrees cannot run the suite at once; that local limitation is outside this change. [Playwright CI guidance](https://playwright.dev/docs/ci), [reporters](https://playwright.dev/docs/test-reporters), [recording options](https://playwright.dev/docs/test-use-options#recording-options)
...
Commit the ruleset body as `.github/rulesets/require-checks.json` and apply it with `gh api repos/sullvn/sull.vn/rulesets --input .github/rulesets/require-checks.json`. GitHub does not read the file; it records the applied policy, the same body can be re-applied after a change or imported through the settings UI, and the integration ID names GitHub Actions as the check source. [Ruleset API](https://docs.github.com/en/rest/repos/rules)
...
| `.github/rulesets/require-checks.json` | Store the ruleset body applied with `gh api`; GitHub does not read it, but it records the policy and can be re-applied or imported. |
...
| `README.md` | Create it. Document Nix and LFS setup, testing, baseline authority and review, report viewing, paired Playwright upgrades, the shell-wide font variable, deployment, and changing a PR’s base. |
...
Keep the README short: give the fresh-clone Nix/LFS/install/test sequence with the direnv shortcut, the baseline-update and saved-report commands, the supported headless Linux arm64 environment, and how the site deploys. Explain that Linux snapshot names omit architecture, retain the minimal-font tradeoff, and cover the maintenance rules above. Add one line to `AGENTS.md` pointing at the README so agents treat `pnpm test` as the contract. Save these instructions before deleting this temporary proposal.

Roll out and verify the implementation in this order:

1. Implement the repository changes and verify the documented Nix/LFS setup from a fresh clone. Run `pnpm test` in Nix. Remove `node_modules/.astro` and `dist/`, then run `pnpm test:visual` alone to verify the build and preview startup fit the server timeout. Confirm a fresh direct Playwright invocation also builds before testing. Check that extra user fonts and preferences cannot alter rendering, and that the date test passes across timezones. Compare the six screenshots against the committed baselines under the new font configuration; zero difference is the expected result, and any difference needs an explanation before regeneration. Preserve and review comparisons before accepting baseline changes. Temporarily introduce a type error and confirm `test:types` reports it, then a build error and confirm the piped server log shows it; restore both.
