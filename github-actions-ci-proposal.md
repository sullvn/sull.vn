...
Reviewed 8 September 2026 · Revised 9 September 2026 after a second review · Proposal only · Temporary review document; remove before merging
...
Make `formatDate()` in `src/utils/date.ts` format dates in UTC, matching `toIsoDay()`. Frontmatter dates are full timestamps such as `2017-11-02T06:30:20.829Z`, and the site defines the calendar day in UTC; that post falls on November 1 by Los Angeles wall clock, so the UTC day is a deliberate content definition. `formatDate()` currently displays the UTC day `2017-11-02` as November 2 in UTC but November 1 in Los Angeles. Those dates are written into the HTML during the build, so changing Playwright’s browser timezone would not fix them. The development host and GitHub’s runners both run in UTC, so this fix protects builds on other machines rather than CI parity. Add one focused test that assigns `process.env.TZ` at runtime, which Node honors, and confirms the same output under UTC and Los Angeles. [Node timezone setting](https://nodejs.org/api/cli.html#tz)
...
Playwright’s console fallback in CI is the terse dot reporter; the `github` reporter is the documented choice for Actions and combines with the HTML reporter. Preserve build-before-preview startup, the preview port, the `--ignore-lock` flag, the foreground-process environment setting, `reuseExistingServer: false`, and all six projects. The environment setting suppresses Astro 7’s agent auto-background mode and the flag skips the preview lock file, so Playwright keeps control of the server. The fixed port and `reuseExistingServer: false` also mean two worktrees cannot run the suite at once; that local limitation is outside this change. [Playwright CI guidance](https://playwright.dev/docs/ci), [reporters](https://playwright.dev/docs/test-reporters), [recording options](https://playwright.dev/docs/test-use-options#recording-options)
...
| Location | Work |
| --- | --- |
| `package.json` | Define the checks with every `test:` script in the `test` chain; keep `test:visual` as `playwright test`. |
| `flake.nix` | Add `pkgs.actionlint` and `pkgs.git-lfs`; assert the Playwright version pairing; move the browser variables into the `env` set; define one fontconfig file that includes the pinned rendering and synthetic rules and ignores host and user preferences. |
| `src/utils/date.ts` and a nearby unit test | Format calendar dates in UTC and verify timezone independence. |
| `playwright.config.ts` | Add focused-test protection, one CI worker, failure traces, the `github` reporter in CI, and a longer server timeout with piped stdout; build with `astro build` before preview. |
...
| `README.md` | Document Nix and LFS setup, testing, baseline authority and review, report viewing, paired Playwright upgrades, deployment, and changing a PR’s base. |
| `AGENTS.md` | Point agents at the README’s test contract. |
| GitHub repository settings | Activate and verify the ruleset; stop npm Dependabot PRs, retain alerts, and check LFS usage and budget. |
...
This revision changes only the proposal. No CI implementation or GitHub settings have been changed. The design assumes `main` is the branch to protect, zero human approvals suit solo maintenance, checks apply to owner merges, npm Dependabot PRs are disabled while alerts remain enabled, and Dependabot maintains the action pins.
